...
-- ---------------------------------------------------------------------------
-- Row Level Security
-- Researchers: full CRUD on their own studies' rows (auth.uid() = owner_id).
-- Participants: no table grants. They use SECURITY DEFINER RPCs with a code.
-- ---------------------------------------------------------------------------
...
-- Participants do not authenticate. They never get table-level SELECT/INSERT
-- on surveys or responses. Login-free access goes through SECURITY DEFINER
-- RPCs (participant_start_session / participant_save_response) which require
-- an active participant_code (and optional study_id / occasion_id).
-- See supabase/migrations/20260910_researcher_auth_participant_rpc.sql.

-- Login-free participant access (SECURITY DEFINER). Researchers use RLS above.
